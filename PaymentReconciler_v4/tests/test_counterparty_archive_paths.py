...
class CounterpartyArchivePathTests(unittest.TestCase):
    def test_counterparty_archive_path_falls_back_to_default(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(cfg.get_counterparty_archive_path("BOA3PTY"), "DEFAULT_ARCHIVE")

    def test_counterparty_archive_path_can_override_default(self):
        with tempfile.TemporaryDirectory() as tmp:
            cfg_path = Path(tmp) / "config.json"
            cfg_path.write_text(json.dumps({
                "archive_path": "DEFAULT_ARCHIVE",
            ...
            self.assertEqual(
                ...
                ["DEFAULT_ARCHIVE", "BOA_ARCHIVE", "CITI_ARCHIVE"],
            )
